...
| `ui` | shared UI harness only; page objects stay in the consumer — see [UI quickstart (partest[ui])](howto-ui.md) |
...
subset of the suite ([Coverage honesty — when the number lies](concepts-coverage-honesty.md)). The deselected count is a set of node ids,
...
When the change landed is in [Migration between partest versions](howto-migration.md).
...
to be, and `ApiClient(domain)` said nothing about it — see [Migration between partest versions](howto-migration.md) for the release
