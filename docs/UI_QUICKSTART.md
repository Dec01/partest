...
|-------|------------|---------|
| **`BasePage`** (default) | **sync** (aqa / pytest-playwright) | `goto`, `expect_visible` — **no await** |
| **`AsyncBasePage`** | async API | `await goto`, `await expect_visible` |

## Isolation rules (LIB-11)
...
mon = attach_monitor(
    ...
    record_all_http_errors=True,  # False = aqa-like asset/API 5xx only
)
...
Default capture is **sync** Playwright (aqa-style). Pass `--async` for the 1.4 async API.
