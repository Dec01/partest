# Migrating to partest 1.5.0

Consumer-proof context: aqa switch issues live in
`MIGRATION_1.3.0_ISSUES.md` (LIB-* closed; remaining = AQA-6).
Operational checklist: [`RELEASE_1.5.0.md`](RELEASE_1.5.0.md).
This file is the **library-side** guide.

Install current:

```bash
pip install -U 'partest>=1.5.0'
pip install -U 'partest[ui]>=1.5.0'   # UI suites
```
...
| **`compare_images(name=, diff_output=)`** | aqa shim kwargs now in library | Drop local visual_compare extras |
...
| **`Storage` class** | aqa-style get/set local/session | `from partest.ui import Storage` |
...
| Change | Impact | Action |
|--------|--------|--------|
| **RiskProfile fields** (LIB-01) | Primary API is now `entity`, `writes`, `authz`, `pii`, `fk_traversal`; levels include **critical** | Prefer new fields; old `name=` / `has_*=` still work; use `RiskProfile.from_legacy(...)` |
| **pytest plugin** (LIB-02) | Still auto entry point; can double-attach with local Allure hooks | Set `PARTEST_PYTEST_PLUGIN=0` or `pytest_plugin = False` |
| **Multi status** (LIB-03) | `expected_status_code=(400, 415)` supported | Update TC that accept either 400 or 415 |
| **Faker locale** (LIB-04) | `PARTEST_FAKER_LOCALE` (default `en_US`) | Set env for RU/DE data |
| **TokenManager** (LIB-05) | `verbose=False` default; logs via `logging` not print | Optional `verbose=True`; optional `client_secret=` |
...
## From aqa-like local harness (wave 1 pattern)
...
### Plugin conflict (aqa root conftest)
...
### Coverage matrix vs flat (LIB-07)
...
### BaseRequestBody property trap (LIB-06)
...
### Collections apply_token (LIB-13/14)
...
- [ ] Full aqa steps: [`RELEASE_1.5.0.md`](RELEASE_1.5.0.md) §4
