...
| **LIB-COV-META** | `meta.merged`, `meta.workers` | ✅ | плюс `partialRun`, `callsTotal`, `unseenRatio` (`null`, если эндпоинтов в прогоне не было); с 1.8.1 — `selection` (`-m`/`-k` прогона) |
...
1. **`LIB-COV-HTML`**, остаток: палитра Ctrl+K и отдельный drawer вместо раскрывающейся
   строки. Обе — удобство, не честность отчёта.
