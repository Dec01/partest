---
title: Testing file upload endpoints
status: current
verified: 2026-09-04
sources: [partest/files.py, partest/client.py]
audience: user
ships_in_wheel: true
---
...
| `request_permissions` | four cells, and the unauthenticated one on a **write** — see [[howto/permissions]] |
...
- Always go through `ApiClient` with `files=`. A raw httpx call carries no `type=` and
  is invisible to coverage — see [[concepts/coverage-honesty]].
...
Related: [[howto/permissions]] · [[howto/recipes]] · [[concepts/methodology]]
